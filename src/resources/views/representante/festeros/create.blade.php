<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">➕ Nuevo Festero</h2>
    </x-slot>

    <div class="py-6">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow sm:rounded-lg">
                @if (session('menor_warning'))
                    <div class="mb-4 rounded-md border border-amber-300 bg-amber-50 text-amber-900 px-4 py-3">
                        {{ session('menor_warning') }}
                        <a href="{{ route('representante.menores.create') }}" class="underline font-medium">Ir a Menores</a>
                    </div>
                @endif

                <form method="POST" action="{{ route('representante.festeros.store') }}" class="space-y-4">
                    @csrf

                    <div class="grid sm:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm">Nombre *</label>
                            <input name="nombre" value="{{ old('nombre') }}" class="border rounded-md px-3 py-2 w-full" required>
                            @error('nombre') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm">Primer apellido *</label>
                            <input name="primer_apellido" value="{{ old('primer_apellido') }}" class="border rounded-md px-3 py-2 w-full" required>
                            @error('primer_apellido') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm">Segundo apellido</label>
                            <input name="segundo_apellido" value="{{ old('segundo_apellido') }}" class="border rounded-md px-3 py-2 w-full">
                            @error('segundo_apellido') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm">DNI</label>
                            <input id="dni" name="dni" value="{{ old('dni') }}" class="border rounded-md px-3 py-2 w-full" autocomplete="off">
                            <p id="dniHelp" class="mt-1 text-sm"></p>
                            @error('dni') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm">Email</label>
                            <input name="email" type="email" value="{{ old('email') }}" class="border rounded-md px-3 py-2 w-full">
                            @error('email') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm">Teléfono</label>
                            <input name="telefono" value="{{ old('telefono') }}" class="border rounded-md px-3 py-2 w-full">
                            @error('telefono') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                        </div>
                        <div class="sm:col-span-2">
                            <label class="block text-sm">Fecha nacimiento</label>
                            <input name="fecha_nacimiento" type="date" value="{{ old('fecha_nacimiento') }}" class="border rounded-md px-3 py-2 w-full">
                    ...
                    <div class="flex items-center gap-6">
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="trabuco" value="1" @checked(old('trabuco'))>
                            <span>Trabuco</span>
                        </label>
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="embarque" value="1" @checked(old('embarque'))>
                            <span>Embarque</span>
                        </label>
                    </div>
                    ...
                    <div class="pt-2">
                        <button class="bg-indigo-600 text-white rounded-md px-4 py-2">Guardar</button>
    ...
    (function() {
        ...
        const ignoreId = ""; // en create no hay ID que ignorar
        ...
        $dni.addEventListener('input', () => {
            ...
            t = setTimeout(async () => {
                try {
                    const params = new URLSearchParams({ dni: raw });
