{{-- welcome.blade.php — Landing segura (cierres Blade simplificados) --}}
<!DOCTYPE html>
...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;800&family=Poppins:wght@400;700;900&display=swap"
        rel="stylesheet">
...
<body class="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 text-white font-body">
    ...
    <header class="relative z-10">
        <div class="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <img src="{{ asset('logo.png') }}" alt="Logotipo Asociación"
                     class="h-10 w-10 rounded-md shadow-lg hidden sm:block">
                <span class="font-display text-xl font-black tracking-tight">
                    {{ config('app.name', 'Moros & Cristianos') }}
                </span>
            </div>
            ...
            <nav class="hidden md:flex items-center gap-6 text-sm">
                ...
                @auth
                    <a href="{{ url('/dashboard') }}"
                       class="inline-flex items-center rounded-xl bg-white/10 px-4 py-2 backdrop-blur hover:bg-white/20 transition">Panel</a>
                @else
                    <a href="{{ route('login') }}"
                       class="inline-flex items-center rounded-xl bg-white text-indigo-900 px-4 py-2 font-semibold hover:translate-y-[-1px] transition">Entrar</a>
                @endauth
            ...
            <div class="grid items-center gap-10 lg:grid-cols-2">
                <div>
                    ...
                    <div class="mt-6 flex flex-wrap gap-3">
                        <a href="#bandos"
                           class="rounded-2xl bg-white text-indigo-900 px-5 py-3 font-semibold shadow hover:shadow-lg transition">Ver comparsas</a>

                        @auth
                            <a href="{{ url('/dashboard') }}"
                               class="rounded-2xl bg-white/10 px-5 py-3 font-semibold backdrop-blur hover:bg-white/20 transition">Ir al panel</a>
                        @else
                            <a href="{{ route('login') }}"
                               class="rounded-2xl bg-pink-400/90 text-indigo-900 px-5 py-3 font-semibold shadow hover:shadow-lg transition">Entrar / Acceder</a>
                        @endauth
                    </div>
                </div>
                        ...
                        <div class="square-content">
                            <div class="h-full w-full rounded-3xl bg-white/10 p-6 backdrop-blur-lg shadow-2xl ring-1 ring-white/20 flex flex-col">
                                ...
                                <div class="mt-4 grid grid-cols-2 gap-3">
                                    <a href="#inscripcion"
                                       class="rounded-xl bg-pink-400/90 text-indigo-900 px-4 py-2 text-center font-semibold hover:bg-pink-300 transition">Inscripción</a>
                                    <a href="#bases"
                                       class="rounded-xl bg-indigo-400/90 text-indigo-900 px-4 py-2 text-center font-semibold hover:bg-indigo-300 transition">Bases</a>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="mx-auto max-w-7xl px-6 py-16">
            ...
            @php
                $moro = [
                    ...
                    'Moros Mercaders','Moros Beduins','Moros de Capeta','Moros Pakkos',
                    'Artillería Mora','Guardia Negra','Negres',
                ];
                $cristiano = [
                    'Pirates Corsaris','Contrabandistes','Pescadors','Artillería Cristiana',
                    'Caçadors','Catalans','Llauradors','Marinos','Destralers','Voluntaris','Almogàvers',
                ];
            @endphp
            ...
            <div class="mt-10 grid gap-6 md:grid-cols-2">
                <div class="rounded-3xl bg-white/10 p-6 backdrop-blur ring-1 ring-white/20 shadow-lg">
                    ...
                    <ul class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2 text-white/90">
                        @foreach ($moro as $c)
                            <li class="rounded-xl bg-white/5 px-3 py-2">🗡️ {{ $c }}</li>
                        @endforeach
                    </ul>
                </div>
                <div class="rounded-3xl bg-white/10 p-6 backdrop-blur ring-1 ring-white/20 shadow-lg">
    ...
    <section id="programa" class="relative z-10 border-t border-white/10">
        <div class="mx-auto max-w-7xl px-6 py-16">
            ...
            <div class="mt-8 grid gap-6 md:grid-cols-3">
                @foreach ([['title' => 'Entrada Mora', 'desc' => 'Desfile principal con boatos y marchas moras.'], ['title' => 'Entrada Cristiana', 'desc' => 'Desfile cristiano con marchas épicas.'], ['title' => 'Embajadas', 'desc' => 'Parlamentos tradicionales y toma del castillo.']] as $item)
                    <div class="rounded-3xl bg-white/10 p-6 backdrop-blur ring-1 ring-white/20 shadow-lg">
    ...
    <footer id="contacto" class="relative z-10 border-t border-white/10">
        ...
        <div class="md:text-right text-white/60 text-sm">
            &copy; {{ date('Y') }} {{ config('app.name', 'Moros & Cristianos') }} · Hecho con Laravel - Vite - TailwindCSS - AlpineJS - Blade - MySQL - PHP 
        </div>
        </div>
    </footer>
    ...
    @guest
        <div class="fixed bottom-4 inset-x-4 z-50 md:hidden">
            <a href="{{ route('login') }}"
               class="block text-center rounded-2xl bg-white text-indigo-900 py-3 font-bold shadow-xl ring-1 ring-black/5">Entrar</a>
        </div>
    @endguest
</body>
</html>
